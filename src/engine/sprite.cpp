...
font_face::font_face(sprite_sheet & sheet, pcf_font_info info) : sheet{sheet}
{  
    for(auto & g : info.glyphs)
    {
        glyphs[g.first].sprite_index = sheet.add_sprite(g.second.bitmap, 0);
        glyphs[g.first].offset = g.second.offset;
        glyphs[g.first].advance = g.second.advance;
    }
