...
{ 
    ...
    constexpr vec dims() const { return {width(), height()}; }
    constexpr floating_point aspect_ratio() const { return static_cast<floating_point>(width())/height(); }

    constexpr rect adjusted(int dx0, int dy0, int dx1, int dy1) const { return {x0+dx0, y0+dy0, x1+dx1, y1+dy1}; }
